...
def session_start(m: Member, channel: str, e: dict):
    ...
    if "SESSION STARTED" in get_pendings(driver=str(m.guild.id), doer=str(m)):
        ...
        stop = write_event_to_db(
            str(m.guild.id), rightnow(), "SESSION ENDED", str(m), True, note
        )
    ...
    pendingID = write_event_to_db(
        str(m.guild.id), rightnow(), "SESSION STARTED", str(m), True, note
    )
...
def session_pause(m: Member, channel: str, e: dict):
    ...
    if "SESSION STARTED" in get_pendings(driver=str(m.guild.id), doer=str(m)):
        ...
        pendingID = write_event_to_db(
            str(m.guild.id), rightnow(), "SESSION PAUSED", str(m), True, note
        )
...
def session_resume(
    ...
    if "SESSION PAUSED" in get_pendings(driver=str(m.guild.id), doer=str(m)):
        ...
        stop = write_event_to_db(
            str(m.guild.id), rightnow(), "SESSION RESUMED", str(m), True, note
        )
...
def channel_change(m: Member, channel: VoiceChannel, e: dict):
    notedic = {"channel": {"name": channel.name, "id": channel.id}}
    notedic = notedic | e
    note = json.dumps(notedic)
    print("CHANNEL CHANGED")
    write_event_to_db(
        str(m.guild.id), rightnow(), "CHANNEL CHANGED", str(m), False, note
    )
...
def talking_start(m: Member, channel: str, e: dict):
    ...
    if "SESSION STARTED" in get_pendings(driver=str(m.guild.id), doer=str(m)):
        ...
        pendingID = write_event_to_db(
            str(m.guild.id), rightnow(), "TALKING STARTED", str(m), True, note
        )
...
def talking_stop(
    ...
    if "TALKING STARTED" in get_pendings(driver=str(m.guild.id), doer=str(m)):
        ...
        note = json.dumps(notedic)
        stop = write_event_to_db(
            str(m.guild.id), rightnow(), "TALKING STOPPED", str(m), True, note
        )
...
def write_pending_to_db(driver: str, doer: str, kind: str, pendingID: int):
    ...
    cur.execute(
        "INSERT INTO pending_event (driver, doer, kind, pendingID) VALUES (%s, %s, %s, %s);",
...
def add_pairid_to_db(start: int, stop: int):
    ...
    cur.execute(
        "SELECT epoch From discord_event WHERE id = ANY(ARRAY[%s, %s]) ORDER BY id;",
        (start, stop),
    )
    epochs = cur.fetchall()
    ...
    cur.execute(
        "UPDATE discord_event SET duration = %s WHERE id = %s", (duration, start)
    )
    cur.execute(
        "UPDATE discord_event SET duration = %s WHERE id = %s", (duration, stop)
    )
...
def get_pair_start_id(driver: str, doer: str, kind: str):
    ...
    cur.execute(
        "SELECT pendingid FROM pending_event WHERE doer = %s AND kind = %s AND driver = %s;",
        (doer, kind, driver),
    )
...
def renew_pendings(driver: str):
    ...
    cur.execute(
        "INSERT INTO discord_event (driver, epoch, kind, doer, isPair) VALUES (%s, %s, %s, %s, %s) RETURNING id;",
    ...
    for each in current_pendings:
        cur.execute(
            "UPDATE discord_event SET pairid = %s WHERE id = %s;",
            (id_of_slash_today_row, each[4]),
        )
        ...
        cur.execute(
            "UPDATE discord_event SET duration = %s WHERE id = %s", (duration, each[4])
        )
...
def renew_pendings_of_a_doer(driver: str, doer: str):
    ...
    cur.execute(
        f"SELECT * FROM pending_event WHERE driver = '{driver}' AND doer = '{doer}'"
    )
    current_pendings = cur.fetchall()
