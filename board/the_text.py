...
import report
from models.Models import InMaahBoard, DiroozBoard
...
async def gen_dirooz_board(guild):
    ...
    DiroozBoard.delete().where(guild_id=guild.id).execute()

    DiroozBoard.create(guild_id=guild.id, channel_id=da_channel.id, msg_id=msg.id, last_update=rightnow())
    # cursor.execute(
...
async def update_dirooz_board(guild):
    ...
    dirooz_row = DiroozBoard.get(DiroozBoard.guild_id == guild.id)
    # db_msg = cursor.fetchone()

    # check if it's outdated
    last_update = JalaliDate.fromtimestamp(dirooz_row.last_update)
    ...
    if JalaliDate.today().day == last_update.day:
        ...
        # get the msg object
        channel = guild.get_channel(dirooz_row.channel_id)
        message = await channel.fetch_message(dirooz_row.msg_id)
        ...
        DiroozBoard.update(last_update=rightnow()).where(guild_id=guild.id).execute()
        # cursor.execute(
...
async def gen_inmaah_board(guild):
    ...
    # InMaahBoard.create(guild_id=guild.id,channel_id=da_channel.id,msg_id=msg.id,last_update=rightnow())
